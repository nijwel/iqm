	...
	<fieldset class="mb-3">
		...
		<div class="row">
			<div class="form-group col-lg-8 mt-2">
				<label class="form-group-lebel">Customer Name <span class="text-danger">*</span></label>
					<select name="customer_id" class="form-control customer_id select22" style="width: 100%" id="customer_id" required>
						...
						<option value="{{ $row->id }}" @if($row->id == $data->customer_id) selected @endif >{{ $row->name }}</option>
						@endforeach
					</select>
			...
			<div class="form-group col-lg-4">
				...
				<input type="number" name="amount" min="0" step="any" value="{{ $data->amount }}" class="form-control" required placeholder="Amount">
			</div>
		</div>
		<!-- /basic text input -->
	</fieldset>
