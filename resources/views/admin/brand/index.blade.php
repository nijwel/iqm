...
@section('content')
    ...
    <div class="page-header page-header-light">
        <div class="page-header-content header-elements-md-inline">
            <div class="page-title d-flex">
                <h4><i class="icon-arrow-left52 mr-2"></i> <span class="font-weight-semibold">Datatables</span> - Brand</h4>
    ...
    <div class="content">
        ...
        <div class="card">
            ...
            <form action="{{ route('brand.multi.delete') }}" method="post" id="multi_delete">
                @csrf
                <table class="table datatable-responsive" id="datatable">
                    ...
                    <tbody>
                        @forelse($data as $key=>$row)
                            <tr>
                                <td><input type="checkbox" name="ids[]" value="{{ $row->id }}" class="checkItem">
                                </td>
                                <td>{{ ++$key }}</td>
                                <td>{{ $row->brand_name }}</td>
                                <td><img src="{{ asset($row->brand_image) }}" width="50"></td>
                                <td>
                                    @if ($row->status == 1)
                                        <a href="{{ route('active.brand', $row->id) }}"><span
                                                class="badge badge-success">Active</span></a>
                                    @else
                                        <a href="{{ route('deactive.brand', $row->id) }}"><span
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                <a href="#" class="dropdown-item edit" data-id="{{ $row->id }}"
                                                    data-toggle="modal" data-target="#editModal"><i
                                                        class="icon-wrench3"></i> Edit</a>
                                                <a href="{{ route('delete.brand', $row->id) }}" id="delete"
                ...
                <div>
                    <button style="display: none;" class="btn btn-danger ml-1 mb-1 delete-btn delete_all" id="delete_btn"><i
                            class="icon-bin mr-2"></i> Delete</button>
                </div>
            </form>
        </div>
        <!-- /basic responsive configuration -->
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form class="form-validate" action="{{ route('store.brand') }}" method="post" id="add_form"
                            enctype="multipart/form-data">
                            @csrf
                            <fieldset class="mb-3">
                                <!-- Basic text input -->
                                <div class="form-group row">
                                    <label class="col-form-label col-lg-12">Brand Name <span
                                            class="text-danger">*</span></label>
                                    <div class="col-lg-12">
                                        <input type="text" name="brand_name" class="form-control"
                                            value="{{ old('brand_name') }}" id="category" placeholder="Brand name">
                                    </div>
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-lg-12">
                                        <input type="file" name="brand_image" class="form-control h-auto" required>
                                    </div>
                                </div>
                                <!-- /basic text input -->
                            </fieldset>
